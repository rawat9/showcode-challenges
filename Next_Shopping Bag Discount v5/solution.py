class Solution:
    ...
    def populate(self, items):
        for item in items:
            ...
            if item_code in self.items_dict:
                self.items_dict.get(item_code)[0] += item_cost
                self.items_dict.get(item_code)[1] += 1
            else:
    ...
    def calculate_bag_total(self, items, discounts):
        ...
        for discount_code in discounts:
            ...
            if (self.items_dict.get(item_code)[1] > min_count):
                continue

            discount = self.__get_discount(item_code, discount_code)
            total_amount += self.items_dict.get(item_code)[0] - discount
